"""
Wrapper to run the [Multi-T RRT planner](https://ieeexplore.ieee.org/abstract/document/6942975) as implemented by the authors of [SFF*](https://github.com/ctu-mrs/space_filling_forest_star) 
"""
...
class MultiTRRT:
    ...
    def __init__(self, numTerminals=5, timeLimit=10, ENV_PATH=TRIANG_ENV, ROBOT_PATH = ROBOT_CYLINDER_SMALL, dateTime="", plannerType=1):
        ...
        self.prob = XMLconfiguration(self.ndim, "rrt", "false")
    ...
    def solve(self):
        ...
        # print("----------\n", res.stdout, "\n----------")
        timeoutLimit = int(self.timeLimit * 1.5)

        try :
            res = subprocess.run(["./" + self.MAIN, "./" + self.DIR + temp], capture_output=True,  text=True, timeout=timeoutLimit)
            print(res.stderr)
            self.parseOutput()
        except subprocess.TimeoutExpired:
            # print("Timeout!!")
            self.timeTaken = timeoutLimit
            self.unsolved()
